...
def test_build_wraps_the_page_in_a_full_document(tmp_path):
    ...
    source.write_text(
        '<title>Zestimator Architecture</title>\n<style>body{}</style>\n\n<div class="wrap">hi</div>\n',
        encoding="utf-8",
    )
    ...
    assert "<title>Zestimator Architecture</title>" in head and "<style>" in head
...
def test_the_real_architecture_page_builds(tmp_path):
    ...
    assert "<title>Zestimator Architecture</title>" in html
