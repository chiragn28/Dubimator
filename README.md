...
- `landing/` — the marketing landing page, live at https://dubimator.vercel.app: Vite, React 19, Tailwind v4 and framer-motion. Run `npm install` then `npm run dev`. The hero video is committed (3 MB, 720p): a drone shot down Sheikh Zayed Road by Mikhail Nilov on Pexels, free for commercial use, cut into a seamless loop. Every outbound link lives in `landing/src/site.ts`; set `VITE_DEMO_URL` at build time to point "Try the Demo" at a public demo instead of `localhost:8501`. To redeploy: `npm run build`, then `vercel deploy dist --prod --yes --name dubimator` from `landing/`.
