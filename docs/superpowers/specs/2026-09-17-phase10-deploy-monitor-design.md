...
| (C) Airflow | A DAG `zestimator_drift`, run manually (the same hand-trigger rule as ingestion), runs the drift report. The Airflow image gains no ML dependencies: the task runs `docker exec zestimator-api python -m monitoring drift` via a BashOperator. Because that needs the Docker socket, this is documented as optional. Simpler default: document a Task Scheduler entry and skip the DAG. **Ruling: no DAG; schedule it with Task Scheduler or cron next to `pipelines retrain`.** |
