...
| (C) Pair model | Phase 4 refits its logistic pair model on every `detect` and never saves it. `python -m listings detect` now also logs it to MLflow as the pyfunc `dubimator-duplicate-pair@champion` (no `code_paths`), holding the pipeline, threshold, feature names and detect run id. A one-off `python -m listings detect` re-run registers the first version. |
